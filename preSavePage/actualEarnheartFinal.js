...
function groupData(options) {
    ...
    for (let i = 0; i < options.data.length; i++) {
        ...
        for (let j = 0; j < options.data[i].StorePurchases.Invoice.length; j++) {
            ...
            if (typeof options.data[i].StorePurchases.Invoice[j].InvoiceDetail.InvoiceDetail[0].GLnumber !== 'string') {
                ...
                if (options.data[i].StorePurchases.Invoice[j].InvoiceDetail.InvoiceDetail[0].VendorName === 'EARNHEART TRANSPORTATION') {
                    // don't delete the earnheart transportation invoices, instead put in its own array for import as a JE
    ...
    for (let i = 0; i < options.data.length; i++) {
        ...
        for (let j = 0; j < options.data[i].StorePurchases.Invoice.length; j++) {
            ...
            for (let k = 0; k < options.data[i].StorePurchases.Invoice[j].Lines.length; k++) {
                // if any lines are 'EARNHEART TRANSPORTATION' (vendor 1303), put in new array
                if (options.data[i].StorePurchases.Invoice[j].Lines[k].VendorNumber === '1303') {
                    options.data[i].earnheartTran.push(options.data[i].StorePurchases.Invoice[j].Lines[k])
                    continue;
                }
    ...
    //delete earnheart transportation arrays from vendor arrays
    for (let i = 0; i < options.data.length; i++) {
        for (let j = 0; j < options.data[i].StorePurchases.Invoice.length; j++) {
